<?php
    ...
    if (isset($_POST['operator'])) {
        $session->setOperator($_POST['operator']);
    }
    if ($session->getGrad() != 0) {
        $id_locatie = $session->getIdLocatie();
    }
    if (isset($_POST['idLocatie'])) {
        $id_locatie = $_POST['idLocatie'];
    }
            ...
            <form method="POST" id="containerDepozit">
                <table class="table-bordered table-striped table-condensed cf col-md-12 tabel-raport zoomable">
                    ...
                    ?>
                                    ...
                                    <td rowspan="2">
                                        <img src='<?php echo  DOMAIN; ?>/images/triangle_<?php echo ($db->getNrEroriAparat($objInfoAparate->idAparat) == 0) ? 'blue' : 'red'; ?>.png' class='eroriPic' serie-Aparat='<?php echo $objInfoAparate->seria ?>' mac-aparat='<?php echo $objInfoAparate->macPic; ?>' data-idAparat='<?php echo $objInfoAparate->idAparat ?>'/>
                                        <?php
                                    ...
                                    <td>
                                        <?php echo $page->getIndexReset($objInfoAparate->lastidxInM); ?>
                                        <?php echo $page->niceIndex($page->verifyIndexLength($objInfoAparate->lastidxInM)); ?>
                                    </td>
                                    <td>
                                        <?php echo $page->getIndexReset($objInfoAparate->lastidxOutM); ?>
                                        <?php echo $page->niceIndex($page->verifyIndexLength($objInfoAparate->lastidxOutM)); ?>
