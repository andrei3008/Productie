                        ...
                        if ($resultAparate = $con->query($queryAparate)) {
                            ...
                            if (isset($informatii)) {
                                for ($i = 0; $i < sizeof($informatii); $i+=2) {
                                    $queryVariabile = "SELECT variabile.fm_mec, pi_mec, MAX(variabile.data) FROM brunersrl.variabile WHERE variabile.idAparat='" . (isset($informatii[$i]->idAparat)) ? $informatii[$i]->idAparat : '' . "' AND variabile.data <= '" . $an . "-" . $luna . "-" . $zi . "'";
