...
if (isset($_GET['zi']) AND isset($_GET['luna']) AND isset($_GET['an'])) {
    $luna = str_replace($invalid_characters, "", $_GET['luna']);
    $an = str_replace($invalid_characters, "", $_GET['an']);
} else {
    $luna = date('m');
    $an = date('Y');
}
...
<html>
    <head>
        <meta charset="UTF-8">
        <title>Raport Lunar <?php echo $infoLocatie->denumire . '-' . $luna . '-' . $an; ?></title>
        <?php require_once('includes/header.php'); ?>
        <link rel="stylesheet" type="text/css" href="css/bootprint.css"/>
        <style>
                    ...
                    <table class="table-bordered table-striped table-condensed cf col-md-12 tabel-raport">
                        ...
                        <?php
                                    $queryMax = "SELECT"
                                            ...
                                            . " contor.idxOutM,"
                                            . " contor.dtServer"
                                            . " FROM brunersrl.contormecanic".$an.$luna." as contor"
                                            . " INNER JOIN brunersrl.aparate "
                                            . " ON contor.idAparat = aparate.idAparat "
                                            . " INNER JOIN brunersrl.locatii ON locatii.idlocatie = aparate.idLocatie "
                                            . " WHERE NOT EXISTS (SELECT * FROM brunersrl.contormecanic201512 as contor2"
                                            . " WHERE contor2.idAparat = contor.idAparat"
                                            . " AND contor2.idxInM > contor.idxInM) "
                                            . " AND contor.idLocatie=".$idLocatie.' AND locatii.operator="'.$_SESSION['operator'].'" ORDER by contor.idAparat ASC';
                                    $queryMin = "SELECT"
                                            . " aparate.idAparat,"
                                            . " aparate.seria,"
                                            . " aparate.idxInM as inInitial,"
                                            ...
                                            . " AND contor2.idxInM < contor.idxInM) "
                                            . " AND contor.idLocatie=".$idLocatie.' AND locatii.operator="'.$_SESSION['operator'].'" ORDER by contor.idAparat ASC';
                        if ($resultMax = $con->query($queryMax) AND $resultMin = $con->query($queryMin)) {
                            $i = 1;
                            $j = 0;
                            while ($objMax = $resultMax->fetch_object()) {
                                $infoSfarsit[$j] = $objMax;
                                $j++;
                            }
                            $j = 0;
                            while ($objMin = $resultMin->fetch_object()) {
                            ...
                            if (isset($infoInceput) AND isset($infoSfarsit)) {
                                for ($i = 0; $i < sizeof($infoInceput); $i++) {
                                    $queryVariabile = "SELECT variabile.fm_mec, pi_mec, MAX(variabile.data) FROM brunersrl.variabile WHERE variabile.idAparat='".$infoInceput[$i]->idAparat."' AND  variabile.data <= '" . $an . "-" . $luna . "-" . date('d') . "'";
